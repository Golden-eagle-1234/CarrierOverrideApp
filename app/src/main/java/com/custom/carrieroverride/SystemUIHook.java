...
public class SystemUIHook implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        XposedBridge.log("[CarrierOverride] Hooking SystemUI with XSharedPreferences");
            ...
            XposedHelpers.findAndHookMethod(subInfoClass, "getCarrierName", new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    try {
                        ...
                        if (!custom.isEmpty()) {
                            XposedBridge.log("[CarrierOverride] SysUI getCarrierName subId=" + subId + " -> " + custom);
                            param.setResult(custom);
                        }
                    } catch (Throwable t) {
                        XposedBridge.log("[CarrierOverride] SysUI error: " + t.getMessage());
                    }
                }
            });
            XposedBridge.log("[CarrierOverride] Hooked SystemUI SubscriptionInfo.getCarrierName");
        } catch (Throwable t) {
            XposedBridge.log("[CarrierOverride] SysUI hook failed: " + t.getMessage());
        }
    }
}
